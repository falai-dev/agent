...
- **Speak call fails**: `turn()` does not throw. The talk step is re-parked under a retry wake with the key `${runId}:${stepId}:${visit}:retry:${atMs}`, at +1m, then +5m, then +15m (and +15m again after that). The outcome is `status: 'deferred'`, `code: 'provider-unavailable'`. The session is saved as usual with `changed: true`, the `do` steps that ran before the call are not repeated, and the retry wake re-runs the same visit, so keys and idempotency hold. When the idle speaker fails instead of a talk step, the outcome is `kind: 'idle'`, `status: 'deferred'`, `code: 'provider-unavailable'` and no wake is scheduled.
