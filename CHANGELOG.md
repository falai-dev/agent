...
- **An `onEnd: 'stay'` flow keeps answering when its last step collects.** A stay run re-entered its last step, and a collect step with every field known is skipped with no call. So once the lead gave the last field, the run sat on that step, skipped it on every message, and held the conversation, so the idle speaker never answered either. The assistant went quiet for good. The same happened when the last step was a `do` or a `say`: the run parked on a step that never talks. Now `'stay'` goes back to the flow's last talk step and answers every message from there, with a new key each time, even with nothing left to collect. The steps after that talk step run once, on the way to the end. `max-asks` is reported once, not on every answer. If the run finishes on a message nobody has answered yet, the talk step answers it in that same turn. A flow with no talk step ends, as with `'end'`. `Run.staying` marks such a run; `assertSession` keeps it. A run the old `'stay'` left on a collect step answers the lead's next message, with no data change. One it left on a `do` or `say` step runs that step once more, with a new key, and then answers.
