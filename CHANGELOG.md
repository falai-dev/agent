...
- **A field the host knows is blank no longer leaves a hole in the sentence.** `{{...}}` had one rule: a path that resolved to nothing kept its braces. That is right for a typo and wrong for a blank value, because an optional field the host fills with `''` is not a mistake — it is a lead with no company. Those two cases are now separate. An unknown path still keeps its placeholder; a blank one drops out and takes the gap with it, so `"Ola {{context.lead.name}}, tudo bem?"` sends `"Ola, tudo bem?"` and never `"Ola , tudo bem?"`. The tidy is narrow on purpose — a doubled space after a visible character, a space before punctuation, a comma left right before the end of a sentence or before another comma (`"Oi, {{context.lead.name}}!"` sends `"Oi!"`, not `"Oi,!"`, and `"Oi, {{context.lead.name}}, tudo bem?"` sends `"Oi, tudo bem?"`, not `"Oi,, tudo bem?"`), punctuation stranded at the start of a line before more text, a space at the end of a line — and it runs only on a string where something actually came back blank, so indentation in a list and an author's own double space survive untouched. v3 did this and v4 had dropped it; the products that render a lead into their copy need it.
...
- **The reply no longer repeats what a `say` just sent.** A flow that sends fixed text and then reaches a talk step does both in one turn, but the speak call only saw `history`, which ends before the turn. So the model never saw the fixed text and said the same thing again: a store's flow sent a fixed message about Android phones, and the AI's reply right after it said it once more. The speak call now lists what this turn already sent (a `say`, a fixed question), in order, under "Already sent", after the customer's message, with the instruction not to repeat it. A turn that sent nothing first adds nothing to the prompt.
...
- **The retry wake answers the customer's question.** After a provider failure, the retry wake spoke with the "you speak first" situation: no new message, do not answer a question nobody asked. But the customer's message was still unanswered, so the model was told to ignore the one thing it had to answer. When a talk step runs again under its retry wake and the host's history still ends with a customer message, the speak call is now told that message has no answer yet. Every other wake still speaks first, because a host may skip a message on purpose, such as a WhatsApp away message, and a follow-up must not answer it. `SpeakRequest.input` carries `retry: true` for that one case.

- **`validateFlow` and `fromSpec` check the JSON's shape before its names.** A stored row or a generated spec with a string where a list belongs crashed with a raw `TypeError` that named no flow (`steps: [null]`, `on: {}`, `then: 5`), or passed and misbehaved: `collect: "nome"` asked for the fields "n", "o", "m", "e", `message: "quer agendar"` reached the model one letter per phrase, `onEnd: "restart"` just ended the run. Each is now a `FlowConfigurationError` that names the flow, the step and the key, with the fix. A step that does two things (`say` and `do`) ran only the first and dropped the other without a word; it is now rejected, and so is a spec step whose `kind` disagrees with its body. A `when` branch on a wait step, which no call ever judges, now logs a warning, and `flowSpecSchema` no longer offers one.

- **The agent build catches two more mistakes that used to fail on live turns, and warns about a third.** An unknown condition in an agent or idle instruction's `if` made every `turn()` throw, and a host condition named `equals`, `known` or `silenced` was never called, because the built-in answered first. Both now throw `FlowConfigurationError` when the agent is built. A literal `then: { flow: "humnao" }` naming no flow is still skipped at run time with `flow-gone`, and the build now logs a warning for it. It stays a warning so that a host which drops one bad row keeps the rest of its agent, including the flows that jump into the dropped one. `Registries` takes an optional `flows` list for that check; a templated `{ flow }` is still resolved per run.
...
- **The speak call fills the templates in a flow's description.** The understand call already rendered `{{context.x}}` and `{{data.x}}` there; the speak call quoted the raw braces to the model that writes the reply.
